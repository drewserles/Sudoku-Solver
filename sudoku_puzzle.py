...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self):
        """
        Return True if the puzzle is impossible to complete in its current
        ...
        False
        """
        # get index of blank locations
        indices = [i for i, x in enumerate(self._symbols) if x == "*"]
        for n in indices:
            # convenient names
            row = self._row_set(n)
            column = self._column_set(n)
            box = self._subsquare_set(n)
            # Missing elements in the row, column, and box of n
            row_miss = [i for i in self._symbol_set if i not in row]
            col_miss = [i for i in self._symbol_set if i not in column]
            box_miss = [i for i in self._symbol_set if i not in box]
            # puzzle is unsolvable when there is only 1 blank space in a
            # row, column, or sub-grid and the required required number exists
            # already in a corresponding row/grid/sub-grid
            if len(row_miss) == 1 and \
                    (row_miss[0] in column or row_miss[0] in box):
                return True
            elif len(col_miss) == 1 and \
                    (col_miss[0] in row or col_miss[0] in box):
                return True
            elif len(box_miss) == 1 and \
                    (box_miss[0] in column or box_miss[0] in row):
                return True
        return False
